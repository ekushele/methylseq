...
# function biscuitQC {
    ...
    # samtools view -hb ${in_bam} | /cs/icore/ekushele/bin/parallel -j7 -k --tmpdir ${outdir} --pipe --tee {} ::: \
        ...
        # "samtools view -F 0x500 -f 0x3 -q 40 -h | /cs/icore/ekushele/bin/biscuit bsconv -b ${genome} - > ${outdir}/${sample}_bsconv.tmp.tsv" \
        # "samtools view -q 40 -h | /cs/icore/ekushele/bin/biscuit cinread ${genome} - -t ch -p QPAIR,CQPOS,CRETENTION | sort -T ${outdir} | uniq -c > ${outdir}/${sample}_cph_ret.tmp.txt" \
        # "samtools view -q 40 -h | /cs/icore/ekushele/bin/biscuit cinread ${genome} - -t cg -p QPAIR,CQPOS,CRETENTION | sort -T ${outdir} | uniq -c > ${outdir}/${sample}_cpg_ret.tmp.txt"

    # # Create cpg_all, cpg_q40
    # cat ${BISCUIT_CPGS} | /cs/icore/ekushele/bin/parallel -j6 -k --tmpdir ${outdir} --pipe --tee {} ::: \
    ...
    # /cs/icore/ekushele/bin/biscuit bsstrand ${genome} ${in_bam} \
    ...
    # if [[ -f ${in_vcf} ]]; then
        ...
        # /cs/icore/ekushele/bin/biscuit vcf2bed -e -t c ${in_vcf} | \
...
function biscuitQC {
    ...
    samtools view -hb ${in_bam} | parallel -j7 -k --tmpdir ${outdir} --pipe --tee {} ::: \
        ...
        "samtools view -f 0x400 -b | bedtools genomecov -bga -split -ibam stdin | LC_ALL=C sort -k1,1 -k2,2n -T ${outdir} > ${outdir}/${sample}_genomecov_all_dup.tmp.bed" \
        "samtools view -f 0x400 -q 40 -b | bedtools genomecov -bga -split -ibam stdin | LC_ALL=C sort -k1,1 -k2,2n -T ${outdir} > ${outdir}/${sample}_genomecov_q40_dup.tmp.bed" \
        "samtools view -F 0x500 -q 40 -h | biscuit bsconv -b ${genome} - > ${outdir}/${sample}_bsconv.tmp.tsv" \
